<?php
    // Zorg dat de databaseverbinding ($conn) beschikbaar is voor de scores
    // Pas de bestandsnaam aan indien nodig
    ...
    /**
   * ============================================================
   * Opstellingsgenerator Voetbal - Hoofdscript (index.php)
   * ============================================================
   */
  ...
  if (file_exists($selectie_path)) {
      ...
      if (isset($format) && isset($sel) && is_array($sel)) {
          ...
          // ------------------------------------------------------------
          // [3b] Live Scores Ophalen uit Database
          // ------------------------------------------------------------
          // We gebruiken de selectie ($sel) die net is ingeladen
          $playerList = $sel; 
          $player_scores = []; // Initialiseer de array

          if (!empty($playerList)) {
              // 1. Haal ID's en Info op voor de spelers in de selectie
              $placeholders = implode(',', array_fill(0, count($playerList), '?'));
              $types = str_repeat('s', count($playerList));

              $sql_players = "SELECT id, shortname FROM players WHERE shortname IN ($placeholders)";
              $stmt = $conn->prepare($sql_players);
              $stmt->bind_param($types, ...$playerList);
              $stmt->execute();
              $result_players = $stmt->get_result();

              $player_ids = [];
              $shortname_map = []; // Om ID terug te koppelen naar Naam

              while ($row = $result_players->fetch_assoc()) {
                  $player_ids[] = $row['id'];
                  $shortname_map[$row['id']] = $row['shortname'];
              }

              // 2. Haal de laatste scores op voor deze ID's
              if (!empty($player_ids)) {
                  $ids_str = implode(',', $player_ids); // Veilig want dit zijn integers uit DB
                  
                  // De query met de MAX(score_date) truc
                  $sql_scores = "SELECT player_id, position, score
                                 FROM player_scores
                  ...
                  while ($row = $result_scores->fetch_assoc()) {
                      $p_name = $shortname_map[$row['player_id']];
                      $p_pos  = $row['position'];
                      $p_score = (int)$row['score'];
                      
                      $player_scores[$p_name][$p_pos] = $p_score;
                  }
              }
          }
   ...
   if ($index === false) {
       ...
       // FIX: We controleren eerst of er wel keys zijn
       if (!empty($keys)) {
   ...
   if (isset($vorige_wedstrijd_key)){
     ...
     $jaar = '20' . substr($datum_raw, 0, 2); 
     $maand = substr($datum_raw, 2, 2);       
     $dag = substr($datum_raw, 4, 2);         
     $datum_full = "$jaar-$maand-$dag";       
     $datum_short = "$dag/$maand";            
  ...
  // ------------------------------------------------------------
    ...
    // DEBUG VARIABELEN INITIALISEREN
    $debug_stats = [
        "duplicates" => 0,
        "max_time_violation" => 0,
        "min_time_violation" => 0,
        "position_violation" => 0,
        "player_violations" => [], // NIEUW: Houdt bij welke speler de fout veroorzaakt
        "valid_lineups" => 0
    ];
    $last_failure_reason = ""; 
    ...
    if ($shuffle_type == "coach"){
      ...
      while (count($usedHashes) < $max_runs && $tries < $max_runs * 10) {
        ...
        if ($max_runs > 1){
          ...
          $list_of_players = array_merge($fixed_players, $shuffled);
          ...
          if (!isset($usedHashes[$hash])) {
              ...
              // -------------------------------------------------------
              // CHECK 1: MAX SPELERS (Die niet het meest mogen spelen)
              // -------------------------------------------------------
              if (isset($wisselschema_meta[$format]['time']['max']) && is_array($wisselschema_meta[$format]['time']['max'])) {
                  if (isset($no_max[$wedstrijd]) && is_array($no_max[$wedstrijd]) && count($no_max[$wedstrijd])) {
                      ...
                      foreach ($teControlerenNamen as $naam) {
                          $positie = array_search($naam, $list_of_players);
                          if ($positie !== false && in_array($positie, $verbodenIndexen)) {
                              $debug_stats["max_time_violation"]++;
                              $last_failure_reason = "MAX Violation: Speler '$naam' staat op positie $positie (is een 'max' positie).";
                              continue 2; 
                          }
                      }
                  }
              ...
              // -------------------------------------------------------
              // CHECK 2: MIN SPELERS (Die niet het minst mogen spelen)
              // -------------------------------------------------------
              if (isset($wisselschema_meta[$format]['time']['min']) && is_array($wisselschema_meta[$format]['time']['min'])) {
                  if (isset($no_min[$wedstrijd]) && is_array($no_min[$wedstrijd]) && count($no_min[$wedstrijd])) {
                      ...
                      foreach ($teControlerenNamen as $naam) {
                          $positie = array_search($naam, $list_of_players);
                          if ($positie !== false && in_array($positie, $verbodenIndexen)) {
                              $debug_stats["min_time_violation"]++;
                              $last_failure_reason = "MIN Violation: Speler '$naam' staat op positie $positie (is een 'min' positie).";
                              continue 2;
                          }
                      }
                  }
              ...
              // -------------------------------------------------------
              // CHECK 3: VERBODEN POSITIES (Speler mag niet op X staan)
              // -------------------------------------------------------
              if (isset($wisselschema_meta[$format]["positions"])) {
                  foreach ($list_of_players as $index => $speler) {
                      ...
                      foreach ($positiesVoorIndex as $positie) {
                          if (in_array($positie, $verbodenPosities)) {
                              // FOUT GEVONDEN
                              $debug_stats["position_violation"]++;
                            
                              // NIEUW: Tel per speler hoe vaak hij de boosdoener is
                              if (!isset($debug_stats["player_violations"][$speler])) {
                                  $debug_stats["player_violations"][$speler] = 0;
                              }
                              $debug_stats["player_violations"][$speler]++;
                            
                              $last_failure_reason = "Position Violation: Speler '$speler' (index $index) komt op positie $positie, maar mag daar niet staan.";
                              continue 3; 
                          }
                      }
                  }
    ...
    if (is_null($result)) {
        echo "<div class='alert alert-danger' style='padding:20px; background-color: #f8d7da; color: #721c24; border: 1px solid #f5c6cb;'>";
        echo "<h3>❌ Geen geldige opstelling gevonden.</h3>";
        echo "<p>Het script heeft <strong>$tries</strong> pogingen gedaan (max_runs: $max_runs).</p>";
      
        echo "<strong>Waarom werden opstellingen afgekeurd?</strong><ul>";
        echo "<li>Duplicate shuffles (toeval): " . $debug_stats["duplicates"] . "</li>";
        echo "<li>MAX Time Violation: " . $debug_stats["max_time_violation"] . "</li>";
        echo "<li>MIN Time Violation: " . $debug_stats["min_time_violation"] . "</li>";
        echo "<li><strong>Position Violation</strong> (Totaal): " . $debug_stats["position_violation"] . "</li>";
        echo "</ul>";

        // NIEUW: Toon top 5 spelers die problemen veroorzaken
        if (!empty($debug_stats["player_violations"])) {
            echo "<strong>Top veroorzakers Position Violations:</strong><br/>";
            echo "<small>(Dit betekent dat deze speler op een plek terechtkwam waar hij score 0 heeft)</small>";
            echo "<ul>";
            arsort($debug_stats["player_violations"]); // Sorteer hoog naar laag
            $count = 0;
            foreach ($debug_stats["player_violations"] as $p => $v) {
                echo "<li><strong>$p</strong>: $v keer</li>";
                $count++;
                if ($count >= 10) break; // Toon max 10
            }
            echo "</ul>";
        }

        if (!empty($last_failure_reason)) {
            echo "<hr><strong>Voorbeeld van de laatste fout:</strong><br/>";
            echo "<code>$last_failure_reason</code>";
        }
      
        echo "<hr><p><em>Tip: Controleer of de spelers bovenaan de lijst niet te veel '0' scores hebben.</em></p>";
        echo "</div>";
        die();
    } else {
  ...
  <main>
    <div class="container">
      ...
      <p class="lead" style="margin-bottom:0">
        <?php 
        echo "$total_players spelers aanwezig: " . implode(", ",array_keys($lineup->playerindex)); 
