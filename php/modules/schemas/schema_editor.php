...
$page_title = "Bewerk Bestaand Schema";
...
<div class="container mt-4 mb-5 pb-5">
    ...
    <div class="row">
        ...
        <div class="col-md-3">
            ...
            <div class="pool-container sticky-top" style="top: 20px; display: none;">
...
function initBuilder() {
    ...
    for(let i=0; i<numShifts; i++) {
        ...
        block.className = 'card h-100 shift-block border-primary';
        block.id = 'shift-' + i;
...
function loadPreloadedSchema(data) {
    ...
    calculateStats();
    document.getElementById('btnSave').disabled = false; // Enabled by default since it's preloaded
}

function createPlayerEl(sidx) {
    let pName = "";
    let isGk = false;
    for(let pid in playersMap) {
...
function handleDrop(e, dropZone) {
    ...
    // Ensure we only swap players WITHIN the same shift
    let sourceShiftIdx = sourceContainer.closest('.shift-block').id.replace('shift-', '');
    if (sourceShiftIdx !== targetShiftIdx.toString()) {
        return; // Reject drag across shifts!
    }
